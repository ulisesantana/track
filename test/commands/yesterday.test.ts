...
describe('yesterday command runs', () => {
  ...
  queryString.set('start_date', yesterday.toISOString().split('T').at(0)!)
  ...
  it('showing yesterday report listing entries grouped by entry description', async () => {
  ...
  it('showing there is no entries for yesterday', async () => {
    ...
    expect(stdoutStub.args.flat().join(',')).to.contains(`There are no entries for yesterday.`)
  })
