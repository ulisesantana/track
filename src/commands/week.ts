import {GetCurrentWeekReportUseCase} from "../application/cases";
...
export default class Today extends TrackCommand {
  static description = 'Show time entries run current week.'
  ...
  public async run(): Promise<void> {
    ...
    const useCase = new GetCurrentWeekReportUseCase(timeEntryRepository)
    ...
    try {
      ...
      if (report.totalDuration.value > 0) {
        ...
        this.log('There are no entries for the current week.')
      }
    } catch (error) {
